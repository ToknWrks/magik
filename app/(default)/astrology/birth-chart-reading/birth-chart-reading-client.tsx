...
// Birth Chart Reading — standalone natal-chart interpretation ($9 or 250 tokens).
// Mirrors the Personal Transit Reading page; NOT the Full Initiation (which
// bundles this reading plus a transit reading for $23).
...
function calculateNatalPositions(birthDate: string, birthTime?: string): Record<string, number> {
  ...
  for (const planet of PLANETS) {
    try { positions[planet] = Astronomy.Ecliptic(Astronomy.GeoVector(planet as any, date, false)).elon; } catch { /* skip */ }
  }
  return positions;
}
...
function ReportDisplay({
  ...
  const natalAspects = findNatalAspects(natalPositions);
  ...
  const openAspectModal = async (a: NatalAspect) => {
    ...
    try {
      const aspectInfo = `Natal ${a.p1} ${a.aspect} Natal ${a.p2} (orb: ${a.orb}°). This is a natal aspect in the birth chart. Use the Archetypal Astrology framework.`;
      const res = await fetch('/api/astrology/interpretations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ transitInfo: aspectInfo }),
      });
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `birth-chart-reading-${reading.birth_date}.txt`;
    a.click();
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Your Birth Chart Reading</h2>
      ...
      {/* Natal aspect cards — tap to explore */}
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mt-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          The fundamental planetary relationships in your birth chart — sorted by intensity. Tap to explore.
        </p>
        <div className="space-y-2">
          {findNatalAspects(calculateNatalPositions(reading.birth_date, reading.birth_time ?? undefined)).slice(0, 8).map((a, ai) => (
            <button
              key={ai}
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{a.orb}° orb · natal aspect</p>
              </div>
...
function PaymentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/astrology/birth-chart-reading', {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div className="max-w-md mx-auto">
      ...
      <div className="mb-6 space-y-1">
        <div className="flex justify-between text-sm">
          <span className="text-gray-500 dark:text-gray-400">Birth Chart Reading</span>
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">
            A personalized archetypal interpretation of your natal chart — your core identity, emotional world, and the architecture of your soul. One-time — $9 or 250 tokens.
          </p>
        </div>
