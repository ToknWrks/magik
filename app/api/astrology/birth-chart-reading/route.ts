...
export async function POST(request: NextRequest) {
  try {
    ...
    // Token price of a Birth Chart Reading (lib/reading-pricing is the source of truth)
    ...
    if (devBypass) {
      ...
      const claimed = await pool.query(
        `UPDATE reading_crypto_payments
         SET claimed_at = NOW()
         WHERE id = $1 AND user_id = $2 AND reading_type = 'birthchart' AND claimed_at IS NULL
         RETURNING id`,
        [cryptoPaymentId, uid]
      );
      ...
      // Enforce the card price server-side (birthchart: card = READING_CARD_USD)
